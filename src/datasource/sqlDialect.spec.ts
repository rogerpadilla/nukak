...
describe.each([MySqlDialect, PostgresDialect])('sqlDialect %p', (Dialect) => {
  ...
  it('transaction begin', () => {
    const command = Dialect === MySqlDialect ? 'START TRANSACTION' : 'BEGIN';
    expect(sql.beginTransactionCommand).toBe(command);
  });
  ...
  it('find $startsWith', () => {
    const query = sql.find(User, {
      filter: { name: 'some name', email: { $startsWith: 'some' } },
      sort: { name: 1, id: -1 },
      skip: 0,
      limit: 50,
    });
    expect(query).toBe(
      "SELECT * FROM `User` WHERE `name` = 'some name' AND LOWER (`email`) LIKE 'some%' ORDER BY `name`, `id` DESC LIMIT 50 OFFSET 0"
    );
  });

  it('find $match', () => {
    const query = sql.find(User, {
      filter: { name: { $match: 'other' }, status: 1 },
      limit: 30,
    });
    expect(query).toBe("SELECT * FROM `User` WHERE MATCH (`name`) AGAINST ('other') AND `status` = 1 LIMIT 30");
  });

  it('find multiple comparison-operators', () => {
    const query1 = sql.find(User, {
      filter: { name: { $match: 'other', $ne: 'other unwanted' }, status: 1 },
      limit: 10,
    });
    expect(query1).toBe(
      "SELECT * FROM `User` WHERE (MATCH (`name`) AGAINST ('other') AND `name` <> 'other unwanted') AND `status` = 1 LIMIT 10"
    );

    const query2 = sql.find(User, {
      filter: { $or: { name: { $startsWith: 'OTHER', $ne: 'other unwanted' }, status: 1 } },
      limit: 10,
    });
    expect(query2).toBe(
      "SELECT * FROM `User` WHERE (LOWER (`name`) LIKE 'other%' AND `name` <> 'other unwanted') OR `status` = 1 LIMIT 10"
    );
  ...
  it('find populates with all fields and specific fields, and filter by populated', () => {
    const qm: Query<Item> = {
      ...
      filter: { 'measureUnit.name': { $startsWith: 'Unidad' }, 'tax.id': 2 } as Item,
    ...
    expect(query).toBe(
      'SELECT `Item`.`id`, `Item`.`name`, ' +
        ...
        'LEFT JOIN `Tax` `tax` ON `tax`.`id` = `Item`.`tax` ' +
        'LEFT JOIN `MeasureUnit` `measureUnit` ON `measureUnit`.`id` = `Item`.`measureUnit` ' +
        "WHERE LOWER (`measureUnit`.`name`) LIKE 'unidad%' AND `tax`.`id` = 2 " +
  ...
  it('find select as functions', () => {
    const query = sql.find(
      User,
      {
        ...
        filter: { name: { $match: 'something' } },
      },
      { trustedProject: true }
    );
    expect(query).toBe(
      'SELECT *, LOG10(numberOfVotes + 1) * 287014.5873982681 + createdAt AS hotness' +
        " FROM `User` WHERE MATCH (`name`) AGAINST ('something')"
    );
  });
