...
describe.each([MySqlDialect, PostgresDialect, SqliteDialect])('%p', (Dialect) => {
  const sql = new Dialect();
  const userUp = sql.escapeId('User');
  const id = sql.escapeId('id');
  const name = sql.escapeId('name');
  const email = sql.escapeId('email');
  const status = sql.escapeId('status');
  const profile = sql.escapeId('profile');
  const company = sql.escapeId('company');
  const user = sql.escapeId('user');
  const createdAt = sql.escapeId('createdAt');
  const updatedAt = sql.escapeId('updatedAt');

  it('escape', () => {
    expect(sql.escape('user')).toBe(`'user'`);
    expect(sql.escape('User')).toBe(`'User'`);
  });
  ...
  it('create', () => {
    ...
    expect(query).toStartsWith(
      `INSERT INTO ${userUp} (${name}, ${email}, ${createdAt}) VALUES` +
  ...
  it('create - one', () => {
    ...
    expect(query).toStartsWith(
      `INSERT INTO ${userUp} (${name}, ${email}, ${createdAt}) VALUES ('Some Name', 'someemail@example.com', 123)`
    );
  });
  ...
  it('update', () => {
    ...
    expect(query).toBe(
      `UPDATE ${userUp} SET ${name} = 'Some Text', ${updatedAt} = 321 WHERE ${name} = 'some' AND ${user} = '123'`
    );
  });
  ...
  it('find', () => {
    ...
    expect(query).toBe(`SELECT * FROM ${userUp} WHERE ${id} = '123' AND ${name} = 'abc'`);
  });
  ...
  it('find $and', () => {
    ...
    expect(quer1).toBe(`SELECT * FROM ${userUp} WHERE ${id} = '123'`);
    const quer2 = sql.find(User, {
      filter: { $and: { id: '123', name: 'abc' } },
    });
    expect(quer2).toBe(`SELECT * FROM ${userUp} WHERE ${id} = '123' AND ${name} = 'abc'`);
    const query3 = sql.find(User, {
      filter: { $and: { id: '123' }, name: 'abc' },
    });
    expect(query3).toBe(`SELECT * FROM ${userUp} WHERE ${id} = '123' AND ${name} = 'abc'`);
  });
  ...
  it('find $or', () => {
    ...
    expect(query1).toBe(`SELECT * FROM ${userUp} WHERE ${id} = '123'`);
    const query2 = sql.find(User, {
      filter: { $or: { id: '123', name: 'abc' } },
    });
    expect(query2).toBe(`SELECT * FROM ${userUp} WHERE ${id} = '123' OR ${name} = 'abc'`);
    const query3 = sql.find(User, {
      filter: { $or: { id: '123' }, name: 'abc' },
    });
    expect(query3).toBe(`SELECT * FROM ${userUp} WHERE ${id} = '123' AND ${name} = 'abc'`);
  ...
  it('find logical operators', () => {
    ...
    expect(query1).toBe(
      `SELECT * FROM ${userUp} WHERE ${user} = '1' AND (${name} IN ('a', 'b', 'c') OR ${email} = 'abc@example.com') AND ${id} = '1'`
    );
    const query2 = sql.find(User, {
      filter: {
        user: '1',
    ...
    expect(query2).toBe(
      `SELECT * FROM ${userUp} WHERE ${user} = '1' AND (${name} IN ('a', 'b', 'c') OR ${email} = 'abc@example.com') AND ${id} = '1' AND ${email} = 'e'`
    );
    const query3 = sql.find(User, {
      filter: {
        user: '1',
    ...
    expect(query3).toBe(
      `SELECT * FROM ${userUp} WHERE ${user} = '1' AND (${name} IN ('a', 'b', 'c') OR ${email} = 'abc@example.com') AND ${id} = '1' AND ${email} = 'e' ORDER BY ${name}, ${createdAt} DESC LIMIT 10 OFFSET 50`
    );
  });
  ...
  it('find single filter', () => {
    ...
    expect(query).toBe(`SELECT * FROM ${userUp} WHERE ${name} = 'some' LIMIT 3`);
  });
  ...
  it('find multiple comparison-operators', () => {
    ...
    expect(query2).toBe(
      `SELECT * FROM ${userUp} WHERE (${name} = 'other' AND ${name} <> 'other unwanted') OR ${status} = 1 LIMIT 10`
    );
    ...
    expect(query3).toBe(`SELECT * FROM ${userUp} WHERE (${createdAt} >= 123 AND ${createdAt} <= 999) LIMIT 10`);

    const query4 = sql.find(User, {
      filter: { createdAt: { $gt: 123, $lt: 999 } },
      limit: 10,
    });
    expect(query4).toBe(`SELECT * FROM ${userUp} WHERE (${createdAt} > 123 AND ${createdAt} < 999) LIMIT 10`);
  });
  ...
  it('find $ne', () => {
    ...
    expect(query).toBe(`SELECT * FROM ${userUp} WHERE ${name} = 'some' AND ${status} <> 5 LIMIT 20`);
  });
  ...
  it('find IS (NOT) NULL', () => {
    ...
    expect(query1).toBe(`SELECT * FROM ${userUp} WHERE ${user} = '123' AND ${status} IS NULL LIMIT 5`);
    const query2 = sql.find(User, {
      filter: { user: '123', status: { $ne: null } },
      limit: 5,
    });
    expect(query2).toBe(`SELECT * FROM ${userUp} WHERE ${user} = '123' AND ${status} IS NOT NULL LIMIT 5`);
  });
  ...
  it('find $in', () => {
    ...
    expect(query).toBe(`SELECT * FROM ${userUp} WHERE ${name} = 'some' AND ${status} IN (1, 2, 3) LIMIT 10`);
  });
  ...
  it('find $nin', () => {
    ...
    expect(query).toBe(`SELECT * FROM ${userUp} WHERE ${name} = 'some' AND ${status} NOT IN (1, 2, 3) LIMIT 10`);
  });
  ...
  it('find populate properties with not fixed type', () => {
    ...
    expect(query).toBe(
      `SELECT ${sql.escapeId(['Item.id', 'Item.name', 'user.id'])} ${sql.escapeId('user.id', true)}, ${sql.escapeId(
        'user.name'
      )} ${sql.escapeId('user.name', true)}, ${sql.escapeId('company.id')} ${sql.escapeId(
        'company.id',
        true
      )}, ${sql.escapeId('company.name')} ${sql.escapeId('company.name', true)} FROM ${sql.escapeId(
        'Item'
      )} LEFT JOIN ${userUp} ${user} ON ${sql.escapeId('user.id')} = ${sql.escapeId(
        'Item.user'
      )} LEFT JOIN ${sql.escapeId('Company')} ${company} ON ${sql.escapeId('company.id')} = ${sql.escapeId(
        'Item.company'
      )}`
    );
  });
  ...
  it('find group', () => {
    ...
    expect(query1).toBe(`SELECT * FROM ${userUp} GROUP BY ${company}`);
    const query2 = sql.find(User, {
      project: { id: 1, name: 1 },
      filter: { status: 1 },
    ...
    expect(query2).toBe(
      `SELECT ${id}, ${name} FROM ${userUp} WHERE ${status} = 1 GROUP BY ${company}, ${profile} ORDER BY ${name} LIMIT 100 OFFSET 50`
    );
  });
  it('find limit', () => {
    const query1 = sql.find(User, {
    ...
    expect(query1).toBe(`SELECT * FROM ${userUp} WHERE ${id} = '9' LIMIT 1`);
    const query2 = sql.find(User, {
      project: { id: 1, name: 1, createdAt: 1 },
      filter: { id: '9' },
      limit: 1,
    });
    expect(query2).toBe(`SELECT ${id}, ${name}, ${createdAt} FROM ${userUp} WHERE ${id} = '9' LIMIT 1`);
    const query3 = sql.find(User, {
      filter: { name: 'something', user: '123' },
      limit: 1,
    });
    expect(query3).toBe(`SELECT * FROM ${userUp} WHERE ${name} = 'something' AND ${user} = '123' LIMIT 1`);
    const query4 = sql.find(User, {
      project: { id: 1, name: 1, createdAt: 1 },
      filter: { user: '123' },
      limit: 25,
    });
    expect(query4).toBe(`SELECT ${id}, ${name}, ${createdAt} FROM ${userUp} WHERE ${user} = '123' LIMIT 25`);
  });
  ...
  it('find project', () => {
    expect(
      ...
      `SELECT ${id}, ${company}, ${user}, ${createdAt}, ${updatedAt}, ${status}, ${name}, ${email}, ${profile} FROM ${userUp}`
    );
    ...
    expect(
      ...
      `SELECT ${id}, ${company}, ${user}, ${createdAt}, ${updatedAt}, ${status}, ${email}, ${profile} FROM ${userUp}`
    );

    expect(
      sql.find(User, {
        project: { id: 1, name: 1, createdAt: 1, password: 0 },
      })
    ).toBe(`SELECT ${id}, ${name}, ${createdAt} FROM ${userUp}`);

    expect(
      sql.find(User, {
        project: { id: 1, name: 0, password: 0 },
      })
    ).toBe(`SELECT ${id} FROM ${userUp}`);
    ...
    expect(
      ...
      `SELECT *, LOG10(numberOfVotes + 1) * 287014.5873982681 + createdAt AS hotness FROM ${userUp} WHERE ${name} = 'something'`
    );
  });

  it('remove', () => {
    const query1 = sql.remove(User, { id: '123' });
    expect(query1).toBe(`DELETE FROM ${userUp} WHERE ${id} = '123'`);
    const query2 = sql.remove(User, { company: '123' });
    expect(query2).toBe(`DELETE FROM ${userUp} WHERE ${company} = '123'`);
  });
});
