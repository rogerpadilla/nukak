...
import { createSpec, Spec } from 'uql/test.util';
import { Query, QueryProject, QuerySort } from 'uql/type';
import { SqlDialect } from '../sqlDialect';
import { MySqlDialect } from './mysqlDialect';

export class MySqlDialectSpec implements Spec {
  sql: SqlDialect;

  beforeEach() {
    this.sql = new MySqlDialect();
  }
  ...
  'find unsupported comparison operator'() {
    expect(() =>
      this.sql.find(User, {
  ...
  'find multiple comparison-operators'() {
    const query2 = this.sql.find(User, {
  ...
  'find populate with projected fields'() {
  ...
  'find populates with all fields and specific fields, and filter by populated'() {
    const qm: Query<Item> = {
      project: { id: 1, name: 1 },
  ...
  'find deep populate with projected fields'() {
  ...
  'find populate properties with not fixed type'() {
    const query = this.sql.find(Item, {
      project: { id: 1, name: 1 },
  ...
  'find populate not annotated field'() {
    expect(() =>
      this.sql.find(Item, {
        project: { id: 1, name: 1 },
