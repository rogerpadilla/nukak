...
export abstract class BaseSqlDialectSpec implements Spec {
  ...
  shouldFindMultipleComparisonOperators() {
    ...
    expect(query4).toBe('SELECT `id` FROM `User` WHERE (`createdAt` > 123 AND `createdAt` < 999) LIMIT 10');
  }
  ...
  shouldFindGroup() {
    ...
    expect(query1).toBe(
      'SELECT `id`, `company`, `user`, `createdAt`, `updatedAt`, `status`, `name`, `email`, `password`, `profile` FROM `User` GROUP BY `company`'
    );
    const query2 = this.sql.find(User, {
      project: { id: 1, name: 1 },
      filter: { status: 1 },
  ...
  shouldFind$re() {
    ...
    expect(query).toBe("SELECT `id` FROM `User` WHERE `name` REGEXP '^some'");
  }
